...
document.addEventListener('DOMContentLoaded', function () {
 ...
 // Ejecutar una vez al cargar por si Django repopuló el form con valores previos
  ...
  // Si el form vino repopulado por Django (después de un submit exitoso o con errores),
  // rellenar también el nombre del auto y el precio en el panel lateral
  (function() {
    var selectModelo = document.getElementById('id_modelo_auto');
    var inputPrecio  = document.getElementById('id_precio_auto');
    if (!selectModelo || !selectModelo.value) return;

    var modelo = selectModelo.value;
    var precio  = inputPrecio ? inputPrecio.value : '';

    var elName  = document.getElementById('informe-car-name');
    var elPrice = document.getElementById('informe-car-price');

    if (elName && modelo)  elName.textContent  = modelo;
    if (elPrice && precio) elPrice.textContent = '$' + parseInt(precio).toLocaleString('es-AR');

    // Marcar visualmente la card seleccionada si el modelo coincide
    document.querySelectorAll('.car-card').forEach(function(card) {
      if (card.dataset.model === modelo) {
        card.classList.add('car-card--selected');
        var btn = card.querySelector('.car-select-btn');
        if (btn) {
          btn.classList.remove('btn--outline');
        ...
        var elImg = document.getElementById('informe-car-img');
        if (elImg && card.dataset.img) {
          elImg.src           = card.dataset.img;
          elImg.alt           = modelo;
          elImg.style.display = 'block';
        }
      }
  ...
  ══════════════════════════════════════════════════════════ */
  var formSimulacion = document.getElementById('simulacion-form');
  if (formSimulacion) {
    formSimulacion.addEventListener('submit', function (e) {
      var inputFecha = document.getElementById('id_fecha_nacimiento');
      var errEl      = document.getElementById('error-fecha-nacimiento');
      ...
      var edadAlFinalizar = edad + cuotas / 12;

      var error = null;
      if (edad < 18) {
        error = 'Debés tener al menos 18 años para solicitar el plan.';
      } else if (edadAlFinalizar > 78) {
        error = 'Con ' + edad + ' años y ' + cuotas + ' cuotas, al finalizar el plan '
              + 'tendrías ' + Math.ceil(edadAlFinalizar) + ' años. El máximo permitido es 78.';
      }
    ...
    if (error) {
        ...
        var btnSubmit = formSimulacion.querySelector('.simulate-btn');
        if (btnSubmit) {
    ...
    // Limpiar error al cambiar fecha o cuotas
    ['id_fecha_nacimiento', 'id_cantidad_cuotas'].forEach(function (id) {
      var el = document.getElementById(id);
...
/* ══════════════════════════════════════════════════════════
   8. LIMPIEZA DE ERRORES EN TIEMPO REAL
══════════════════════════════════════════════════════════ */
document.querySelectorAll('.form-input').forEach(function(input) {
  ['input', 'change'].forEach(function(evento) {
    input.addEventListener(evento, function() {
      // Si el campo tiene valor, ocultar el div de error asociado
      if (input.value && input.value.trim() !== '') {
        var grupo = input.closest('.form-group');
        if (grupo) {
          var errorDiv = grupo.querySelector('.form-error');
          if (errorDiv) {
            errorDiv.style.display = 'none';
          }
          input.classList.remove('input--error'); // si usás esta clase
        }
      }
    });
  });
